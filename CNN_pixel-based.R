...
########### U-NET FOR PIXEL-BASED ANALYSIS ###########
...
vgg16_feat_extr <- application_vgg16(include_top = F,
                                     input_shape = c(128,128,3), # TODO change shape?
                                     weights = "imagenet")
...
######### PRETRAINED VGG16 COMBINED WITH A DIFFERENT OWN U-NET #########
...
combined_unet_model <- keras_model(inputs = vgg16_feat_extr$input,
                                   outputs = unet_tensor)
